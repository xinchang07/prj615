---
title: "615_final"
subtitle: "twiiter mining for movie Moana"
author: "Xin Chang"
...
```{r}
#The code I use to get data from twiiter and clean it is as following:(do not run)
# library(ROAuth)
# library(streamR)
...
From following graph, we could know that when people tweets about the movie.
...
cleanText<- removeWords(cleanText,c(stopwords("english"),remwords))
remwords2 <- c("movie","see","watch","just","moana","te","moanas","amp") #without deleting the film name
cleanText2<- removeWords(cleanText,c(stopwords("english"),remwords2))
corpus <- Corpus(VectorSource(cleanText2))
...
```{r}
#Look at the words more clearly with frequency more than 50
...
It is found that 
```{r}
frozen<-length((grep("Frozen", tweets_movie.df$text, ignore.case = TRUE)))
elsa<-length((grep("elsa", tweets_movie.df$text, ignore.case = TRUE)))
print(c("Number of tweets mention Elsa:",elsa))
print(c("Number of tweets mention Frozen:",frozen))
```
...
pol =
  lapply(text.df$text, function(txt) {
...
text.df$emotionalValence = sapply(pol, function(x) x$all$polarity)


# As reality check, what are the most and least positive tweets
text.df$text[which.max(text.df$emotionalValence)]
text.df$text[which.min(text.df$emotionalValence)]
neutral <- length(which(text.df$emotionalValence == 0))
positive <- length(which(text.df$emotionalValence > 0))
negative <- length(which(text.df$emotionalValence < 0))
...
#Locate the positive and negative reviews

Now let's look at the locations of these tweets again, but this time we focus on the spread of positive and negative tweets. From the above part, we know that most of the tweets are positive, this pattern also shows at this graph. And we could found that there is no significant spatial patterns regarding emotional valence. 

```{r}
emotion.df<-text.df %>%
select(lon,lat,emotionalValence)
n<-length(emotion.df$emotionalValence)
emotion.df$group<-rep(NA,n)
emotion.df$group[tweets_movie.df$emotionalValence > 0]<- "positive"
emotion.df$group[tweets_movie.df$emotionalValence ==0]<- "neutral"
emotion.df$group[tweets_movie.df$emotionalValence < 0]<- "negative"

map.data <- map_data("state")
points <- data.frame(x = as.numeric(emotion.df$lat),
                       y=as.numeric(emotion.df$lat))
points$group<-emotion.df$group
points<-na.omit(points)
points<-points %>%
  filter(y>-25 , y< 50)
points<-points %>%
...
ggplot(map.data) + geom_map(aes(map_id = region), map = map.data, fill = "white",
  ...
  theme(axis.line = element_blank(), axis.text = element_blank(), axis.ticks = element_blank(),
        axis.title = element_blank(), panel.background = element_blank(), panel.border = element_blank(),
        panel.grid.major = element_blank(), plot.background = element_blank(), plot.margin = unit(0 * c(-1.5, -1.5, -1.5, -1.5), "lines"))+
  geom_point (data = points, aes(x = x, y = y,fill = factor(group)) ,size =2,shape = 21)
```
